...
# Hugging Face yerine McAuley Lab server
BASE_REPO = "https://mcauleylab.ucsd.edu/public_datasets/data/amazon_2023/raw"
...
def process_category(category, min_reviews=50, min_rating=3.8, limit=1000, n_each=10):
    ...
    # UCSD server linklerini ayarla
    url_reviews = f"{LOCAL_DIR}/{category}.jsonl"
    if not os.path.exists(url_reviews):
        url_reviews = f"{BASE_REPO}/review_categories/{category}.jsonl.gz"

    url_meta = f"{LOCAL_DIR}/meta_{category}.jsonl"
    if not os.path.exists(url_meta):
        url_meta = f"{BASE_REPO}/meta_categories/meta_{category}.jsonl.gz"
